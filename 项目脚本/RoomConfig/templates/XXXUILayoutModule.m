//
//  __Prefix__UILayoutReqeuestModule.m
//  __Prefix__Room
//
//  Created by MOMO on __year__.
//

#import "__Prefix__UILayoutModule.h"
...
@interface __Prefix__RoomContainer : UIView

@end

@implementation __Prefix__RoomContainer
...
@interface __Prefix__UILayoutModule ()

@end

@implementation __Prefix__UILayoutModule
...
- (void)configBaseUIViews:(MBCommonModuleContext *)moduleContext
{
    ...
    _containerView = [[__Prefix__RoomContainer alloc] initWithFrame:superViewBounds];
...
- (UIView *)actionView
{
    if (!_actionView) {
        ...
        _actionView = [[__Prefix__RoomContainer alloc] initWithFrame:superView.bounds];
...
- (UIView *)alertView
{
    if (!_alertView) {
        UIView *superView = MBCommonGetModuleMediator.moduleContext.inView;
        _alertView = [[__Prefix__RoomContainer alloc] initWithFrame:superView.bounds];
        if (_inputView) {
...
- (UIView *)giftEffectView
{
    if (!_giftEffectView) {
        UIView *superView = MBCommonGetModuleMediator.moduleContext.inView;
        _giftEffectView = [[__Prefix__RoomContainer alloc] initWithFrame:superView.bounds];
        if (_alertView) {
...
- (UIView *)inputView
{
    if (!_inputView) {
        UIView *superView = MBCommonGetModuleMediator.moduleContext.inView;
        _inputView = [[__Prefix__RoomContainer alloc] initWithFrame:superView.bounds];
